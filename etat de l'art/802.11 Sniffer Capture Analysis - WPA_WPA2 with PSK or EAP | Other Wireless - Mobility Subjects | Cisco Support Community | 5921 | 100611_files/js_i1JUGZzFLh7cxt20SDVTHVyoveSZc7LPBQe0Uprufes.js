!function(){jQuery.subscribe("cisco_esi_panels_actions.empty",function(){var c=jQuery("div.cisco-actions").html();""===c&&jQuery("div.cisco-actions").parents(".pane-content").parent().hide()})}(jQuery);
;
(function ($) {

Drupal.behaviors.activityStreamToggleDetails = {
  attach: function(context, settings) {

    $(".activity-simple-toggle").unbind("click");
    $(".activity-simple-toggle").click(function(e) {
      e.preventDefault();
      $(".view-cisco-activity-stream-user .views-row .message-body").toggle("hidden");
      if( $("a", this).html() == Drupal.t("Show full list")) {
        $("a", this).html(Drupal.t("Show simple list"));
      } else {
        $("a", this).html(Drupal.t("Show full list"));
      }
    });
    
    //Change values on dropdowns
    $("#edit-type option[value=All]").text("All");
    $("#edit-type-2 option[value=All]").text("All");
    
    if($("#edit-type").val() == "discussion"){
     $("#edit-type-2 option[value=2]").text("Replied");
    }else{
     $("#edit-type-2 option[value=2]").text("Commented");
    }
    
    //Spinner for profile tabs
    $(".view-cisco-activity-stream-user .views-exposed-widgets .ajax-progress").remove();

    if ('undefined' !== typeof($(".view-cisco-activity-stream-user select:not(.cisco-processed)").on)) {
      $(".view-cisco-activity-stream-user select:not(.cisco-processed)").on("change",function(event){
        $(".views-exposed-widgets").prepend('<div class="ajax-progress ajax-progress-throbber"><div class="throbber">&nbsp;</div></div>');
      }).addClass("cisco-processed");
    }
  }
}
...
(function ($) {
Drupal.behaviors.flipboardAskQuestionValidate = {
  attach: function(context, settings) {
    /**
     * Confirm discussion post and set errors if any exist.
     *
     * @param {array} errors
     ...
     */
    $.fn.confirmDiscussionPost = function(errors, values) {
      $('.control-group').removeClass('error');

      if ((errors !== undefined) && errors.length > 0) {
        var errorMsgs = '<ul>';
        for (var i = 0; i < errors.length; i++) {
          $('.form-item-' + errors[i]['key']).addClass('error');
          errorMsgs += '<li>' + errors[i]['message'] + '</li>';
        }
        errorMsgs += '</ul>';

        // Show error modal
        $('#ask-question-error-modal .modal-body').html(errorMsgs);
        $('#ask-question-error-modal').appendTo("body").modal('show');
      }
      else {
        // Hide ask question form on success.
        $('.ask-question-form').hide();

        // Show the filter view again.
        $('#pane-ask-question').show();

        // Insert question information into confirmation modal
        $('#ask-question-confirm span').html(values['community_name']);
        $('#ask-question-confirm-modal .discussion-title').html(values['title']);
        $('#ask-question-confirm-modal .discussion-body').html(values['body']);
        $('#ask-question-confirm-modal .discussion-community span').html(values['community']);
        $('#ask-question-confirm-modal .discussion-tags span').html(values['tags']);
        $('#ask-question-confirm-modal .discussion-attach').html(values['attach']);
        // Show confirmation modal
        $('#ask-question-confirm-modal').appendTo("body").modal('show');
        // Redirect to new discussion page
        $('#ask-question-confirm-modal .btn-primary').click(function() {
            window.location.href=values['url'];
        });
      }
    }

    $.fn.previewDiscussionPost = function() {
      // Add values to preview modal.
      $('#ask-preview-title').html($('input[id*="edit-title-field"]').val());
      $('#ask-preview-body').html($('iframe.cke_wysiwyg_frame').contents().find('body').html());

      // Show preview modal.
      $('#ask-question-preview-modal').appendTo("body").modal('show');
    }
  }
}
})(jQuery);
...
Drupal.behaviors.flipboard = {
  attach: function(context, settings) {
    // Behavior for flipboard tabs.
    $('#flipboard-tabs a').click(function (e) {
      e.preventDefault();
      $(this).tab('show');
    });
  }
...
// prevent submitting a question twice
Drupal.behaviors.flipboardPreventDoublePost = {
	attach: function(context, settings) {
		$('#pane-ask-question').submit(function(){
			$('button', this).attr('disabled','disabled');
		});
	}
}

Drupal.ajax.prototype.commands.fbQuestionAjax = function(ajax, response, status)
{
	var header = $('#pane-ask-question .question-head-wrapper h3');
  var header_text = $('#pane-ask-question .question-head-wrapper p');
	var filterDiv = $("#cisco-flipboard .view-ask-a-question .view-filters");
	var textBox = $("#pane-ask-question #edit-search-box");

	$(header_text).hide();
  $(header).css('margin', 0);

  // Retrieve the title of the page and add it to the button.
  var url = $.param.querystring($('.flipboard-ask-question-btn').attr('href'), 'title=' + $("#pane-ask-question #edit-search-box").val())
  $('.flipboard-ask-question-btn').attr('href', url);

  // Show the button.
  $('.flipboard-ask-question-btn').removeClass("hide");

  // Add spinner while loading view.
  // $('.view-ask-a-question').ajaxStart(function() {
  //   // Append loading div while live filter is running.
  //   $('#pane-ask-question #edit-title-wrapper .controls').append('<div id="loading"></div>');
  //   $('.flipboard-ask-question-btn').prop('href', '#');
  // });
}

Drupal.ajax.prototype.commands.fbShowQuestionForm = function(ajax, response, status) {
    // Fill in title on discussion form.
    $('.ask-question-form .field-name-title-field input').val($("#pane-ask-question #edit-search-box").val());
    if ($("#quicktabs-community_questions").length) {
        $('html, body').scrollTop($("#quicktabs-community_questions").offset().top - 50);
    }
    if ($('#pane-ask-question .pane-content').length) {
        $('#pane-ask-question .pane-content').hide();
    } else {
        $('#pane-ask-question').hide();
    }
    $('.ask-question-form').show();

    // Behavior for Cancel button on Ask Question form.
    $('.ask-question-cancel-btn').click(function (e) {
        if ($('#pane-ask-question .pane-content').length) {
            $('#pane-ask-question .pane-content').show();
        } else {
            $('#pane-ask-question').show();
        }
        e.preventDefault();
        ...
        $('.ask-question-form').html('');
        $('#discussion-node-form')[0].reset();
        // Reset ask question wysiwyg editor
        $('#cke_edit-body-und-0-value iframe').contents().find('body.cke_editable').html('');
    });
}

Drupal.behaviors.toggleDetails = {
	attach: function(context, settings) {
		...
		$(".toggle-details").click(function(e) {
			e.preventDefault();
			$(".view-ask-a-question .views-row .body").toggle("hidden");
			if( $("a", this).html() == Drupal.t("Show detailed list")) {
				$("a", this).html(Drupal.t("Show simple list"));
			} else {
				$("a", this).html(Drupal.t("Show detailed list"));
			}
		});
	}
}
...
/**
 * @file
 * Code for ctools modals.
 */

(function($) {
  /**
   * Override the default CTools Modal Dialog.
   */
  Drupal.theme.prototype.CiscoModalDialog = function () {
    var html = ''
        html += '  <div>' // wrapper placeholder that will be replaced as the modal loads.
        html += '    <div class="modal cisco-messaging-modal">' // panels-modal-content
        html += '      <div class="modal-header">';
        html += '        <a class="ctools-close-modal pull-right" href="#">';
        html += '        <span>  ' + Drupal.t('Close');
        html += '        </a></span>';
        html += '        <h3 id="modal-title"> </h3>';
        html += '      </div>';
        html += '      <div class="modal-body">';
        html += '        <div class="inside"><div id="modal-content" class="modal-content"> </div></div>';
        html += '      </div>';
        html += '    </div>';
        html += '  </div>';
...
( function ($) {
  /**
   * Menu actions for cisco_profile.
   */
  Drupal.behaviors.cisco_profile_menu = {
    attach: function(context,settings) {

      // Reuse user menu block in both regions
      setUserMenuContext = function() {
        // Retrieve the data.
        var data = $('.visible-desktop .block-cisco-profile').data('user-menu'),
        mobile = $('.hidden-desktop .block-cisco-profile'),
        desktop = $('.visible-desktop .block-cisco-profile');

        // Mobile.
        if ($('.hidden-desktop').is(':visible')) {
          // Populate mobile if empty.
          if (mobile.is(':empty')) {
      ...
      $('.visible-desktop .block-cisco-profile').once('user-menu', function() {
        // Retrieve the login menu.
        $(this).data('user-menu', $(this).html());

        // Empty the mobile div.
        $('.hidden-desktop .block-cisco-profile').empty();

        // Set the user context.
        setUserMenuContext();
      });

      // Set user menu resize.
      $('body').once('user-menu-resize', function() {
        $(window).resize(setUserMenuContext);
      });
    }
  }
   ...
   */
  Drupal.behaviors.cisco_profile_notification = {
    attach: function(context,settings) {
      // Notification actions.
      $('body').once('notification-actions', function() {
        $(document).bind('flagGlobalAfterLinkUpdate', function(event, data) {
          if (data.updateCounter) {
            // Update Following counts.
            var uid = data.updateUID + "";
            if ($(".page-user .cisco-profile-following-count-" + uid).length) {
            var followcountstr = $(".cisco-profile-following-count-" + uid).html();
            // Increment or decrement based on type of flag.
            var followcountint = parseInt(followcountstr) + data.updateValue;
            followcountstr = followcountint + "";
            // Update modal window.
            $(".cisco-profile-following-count-"+uid).html(followcountstr);
            // Update social block on profile.
            $(".cisco-profile-social-wrapper-" + uid + " .cisco-profile-following-count").html(followcountstr);
          }

          // Update Followers counts.
          var uid = data.contentId + "";

          if ($(".page-user .cisco-profile-followers-count-" + uid).length) {
            var followcountstr = $(".cisco-profile-followers-count-" + uid).html();
            // Increment or decrement based on type of flag.
            var followcountint = parseInt(followcountstr) + data.updateValue;
            followcountstr = followcountint+"";
            // Update modal window.
            $(".cisco-profile-followers-count-" + uid).html(followcountstr);
            // Update social block on profile.
            $(".cisco-profile-social-wrapper-" + uid + " .cisco-profile-followers-count").html(followcountstr);
          }

          // Prevent additional updates until the next flag operation
        ...
        $(document).bind('flagGlobalAfterLinkUpdate', function(event, data) {
           // Hide all message classes.
           $('.message-' + data.contentId).hide();
         });
      });

    // Change text to removing when clicked.
    $('.view-cisco-notification-user-notifications .flag-cisco-visible-message a').once('remove-notification', function() {
      // Hide on click.
      $(this).click(function() {
        // Change to processing message.
        $(this).hide();
        $(this).after('<a class="btn" href="#">' + Drupal.t('Removing') + '...' + '</a>');
      });
    });
  }
  };
  ...
  /**
   * Login link destination for anonymous users.
   */
  Drupal.behaviors.cisco_profile_login_link = {
    attach: function (context, settings) {
      // Set login click action.
      var login_click = function(event) {
        var path = document.location.pathname.substring(1);

        // Path found.
        if (path.length > 0) {
          $(".js-login-link").querystring({destination: path}, 1);
        }
      };

      $(".js-login-link").once('login-link-destination', function() {
        $(this).click(login_click);
      });
    }
  }
...
;
(function ($) {
  // you can enable debug by entering this command in chrome or firefox's console
  // jQuery.cookie('survey_debug', 'true', {path:'/'})
  //
  // you can remove it by entering
  // jQuery.cookie('survey_debug', null, {path:'/'})
  var survey_debug = false;
  if ($.cookie('survey_debug') !== null){
    survey_debug = true;
  }

  // remove all cookies for debug
  if (survey_debug === true){
    $.cookie('survey_date', null, {path: '/'});
    $.cookie('survey_decline', null, {path: '/'});
  }

  var timestamp = new Date().getTime();

  // set the cookie lifetime to 1000 days
  var cookieLifetime = 1000 * 60 * 60 * 24 * 1000;

  var getHeightFromTop = function () {
    var heighFromTop = window.pageYOffset;

    //check IE
    if((typeof(heightFromTop) === 'undefined') || (heightFromTop === null)){
      heightFromTop = (document.documentElement.scrollTop?document.documentElement.scrollTop:document.body.scrollTop);
    }

    return heightFromTop;
  }

  survey_eligible = function (context, settings){
    // only show the survey X percent of the time.
    // for example: 0.01 will mean 1% of the people get surveys
    var popupFrequency = parseInt(settings.cisco_core.popupFrequency);
    if (Math.random() > (1/popupFrequency)){
      if (survey_debug === true){
        console.log('RNG would have blocked, but we are going forward anyway.');
      }
      else {
        return false;
      }
    ...
    // let's make sure they aren't on an iphone
    if($(window).width() < settings.cisco_core.surveyMinWidth) {
      if (survey_debug === true){
    ...
    // we check if they have declined the survey and whether it's been longer than specified wait period
    if ($.cookie('survey_decline') !== null){
      if (timestamp < ($.cookie('survey_decline') + settings.cisco_core.surveyRetakeDelay)){
        if (survey_debug === true){
        ...
        // clear the declined cookie
        $.cookie('survey_decline', null, {path: '/'});
        return true;
      }
    }

    var survey_date = $.cookie('survey_date');

    // if the user wasn't surveyed, they can be
    if (survey_date === null){
      if (survey_debug === true){
        console.log('not yet taken');
      }
      return true;
    }

    // if they were, but it has been more than the specified wait period, they can be
    if (timestamp > (survey_date + settings.cisco_core.surveyRetakeDelay)){
      if (survey_debug === true){
        console.log('more than ' + settings.cisco_core.surveyRetakeDelay + ' milliseconds');
      }
      return true;
    }
    else {
      if (survey_debug === true){
        console.log('fewer than ' + settings.cisco_core.surveyRetakeDelay + ' milliseconds');
      }
      return false;
    }
  }
  ...
  closeSurveyWindow = function () {
    $("div#lb_overlay").hide();
    $("div#lb_container_usersurvey").hide();
    $("div#lb_container_usersurvey_next_screen").hide();

    var heightFromTop = getHeightFromTop();

    if(heightFromTop > 150) {
  ...
  /* start the survey*/
  function startSurvey(context, settings) {
    // check if the survey should be shown
    if(!(survey_eligible(context, settings))) {
      return;
    }

    var surveyRenderNewWindow = true;
    if (settings.cisco_core.surveyTarget === 'overlay'){
      surveyRenderNewWindow = false;
    }
    var surveyTitle = " Cisco Support Forums Survey ";
    var surveyText = " Please help us improve your Cisco Support Community experience by giving us feedback ";

    var browserLanguage = settings.cisco_core.language || (window.navigator.userLanguage || window.navigator.language);
    browserLanguage = browserLanguage.substring(0,2);

    var languageStrings = {
      en: {
        request : 'Please help us improve your Cisco Support Community experience by giving us feedback',
        acceptsurvey : 'OK',
        acceptsurveyoverlay : 'Click here to participate in the survey',
        acceptcancel : 'Not Today',
        acceptcanceloverlay : 'Close the window',
        title : 'Cisco Support Forums Survey'
      },
      pl : {
        request : 'Prosz\u0119 pom\u00f3\u017c nam ulepsza\u0107 Cisco Support Community dodaj\u0105c swoj\u0105 opini\u0119',
        acceptsurvey : 'OK',
        acceptsurveyoverlay : 'Kliknij tutaj by wzi\u0105\u0107 udzia\u0142 w ankiecie',
        acceptcancel : 'Nie dzisiaj',
        acceptcanceloverlay : 'Zamknij to okno',
        title : 'Ankieta na Forum Cisco Support'
      },
      es : {
        request : 'Por favor ay\u00fadenos a mejorar su experiencia en la Comunidad de Soporte de Cisco mediante esta encuesta',
        acceptsurvey : 'OK',
        acceptsurveyoverlay : 'Selecciona aqu\u00ed para participar en la encuesta',
        acceptcancel : 'Hoy no',
        acceptcanceloverlay : 'Cerrar la ventana',
        title : 'Encuesta de la comunidad de soporte de Cisco'
      },
      ja : {
        request : '\u30b5\u30dd\u30fc\u30c8\u30b3\u30df\u30e5\u30cb\u30c6\u30a3\u306e\u5411\u4e0a\u306e\u305f\u3081\u56de\u7b54\u3078\u306e\u3054\u5354\u529b\u3092\u304a\u9858\u3044\u3044\u305f\u3057\u307e\u3059\u3002',
        acceptsurvey : '\u56de\u7b54\u3059\u308b',
        acceptsurveyoverlay : '\u30af\u30ea\u30c3\u30af\u3057\u3066\u30a2\u30f3\u30b1\u30fc\u30c8\u306b\u56de\u7b54\u3059\u308b\u00a0',
        acceptcancel : '\u4eca\u56de\u306f\u3057\u306a\u3044',
        acceptcanceloverlay : '\u9589\u3058\u308b',
        title : '\u30b7\u30b9\u30b3\u30b5\u30dd\u30fc\u30c8\u30b3\u30df\u30e5\u30cb\u30c6\u30a3\u30a2\u30f3\u30b1\u30fc\u30c8'
      },
      pt : {
        request : 'Por favor nos ajude a melhorar a sua experi\u00eancia na Comunidade de Suporte Cisco nos dando seu feedback',
        acceptsurvey : 'OK',
        acceptsurveyoverlay : 'Clique aqui para participar da pesquisa',
        acceptcancel : 'Hoje n\u00e3o',
        acceptcanceloverlay : 'Fechar a janela',
        title : 'Pesquisa da Comunidade de Suporte Cisco'
      },
      ru : {
        request : '\u041f\u043e\u043c\u043e\u0433\u0438\u0442\u0435 \u043d\u0430\u043c \u0443\u043b\u0443\u0447\u0448\u0438\u0442\u044c Cisco Support Community \u043e\u0441\u0442\u0430\u0432\u0438\u0432 \u0441\u0432\u043e\u0439 \u043e\u0442\u0437\u044b\u0432',
        acceptsurvey : 'OK',
        acceptsurveyoverlay : '\u041d\u0430\u0436\u043c\u0438\u0442\u0435, \u0447\u0442\u043e\u0431\u044b \u043f\u0440\u0438\u043d\u044f\u0442\u044c \u0443\u0447\u0430\u0441\u0442\u0438\u0435 \u0432 \u043e\u043f\u0440\u043e\u0441\u0435',
        acceptcancel : '\u041d\u0435 \u0441\u0435\u0433\u043e\u0434\u043d\u044f',
        acceptcanceloverlay : '\u0417\u0430\u043a\u0440\u044b\u0442\u044c \u043e\u043a\u043d\u043e',
        title : '\u041e\u043f\u0440\u043e\u0441 \u0441\u043e\u043e\u0431\u0449\u0435\u0441\u0442\u0432\u0430 \u043f\u043e\u0434\u0434\u0435\u0440\u0436\u043a\u0438 Cisco'
      }
    };

    var surveyLanguageID;
    var country;

    switch (browserLanguage) {
      case 'ja':
        surveyLanguageID = '6A5348A70B8AA0B5';
        country = 'ja-JP';
        break;
      case 'es':
        surveyLanguageID = '6A5348A70B8AA0B5';
      ...
      default:
        surveyLanguageID = '6A5348A70B8AA0B5';
        country = '';
        // we need the browser language to be one of the listed options, so we set this to english if no matching ones are found. This allows our translated strings to work below.
        browserLanguage = 'en';
        break;
    }

    var voviciSurveyLink = 'https://www.ciscofeedback.vovici.com/se.ashx?s=' + surveyLanguageID + '&c=' + country + '&communityId=' + Drupal.settings.cisco_core.surveyCommunity;

//    var voviciSurveyLink = 'https://www.ciscofeedback.vovici.com/se.ashx?s=' + surveyLanguageID + '&communityId=' + Drupal.settings.cisco_core.surveyCommunity;

    $("div#lb_container_popup_survey").replaceWith('<div id="lb_container_usersurvey" style="width: 100%; top: 0px; left: 0px; right: 0px; bottom: 0px; z-index: 999; display: none"><div id="usersurvey-modal" class="jive-modal" style="position: absolute; left: 60%; width:300px; margin-left: -354px; z-index: 1000;display: block;"><div><span><h2 class="jive-modal-title" style="width: 280px"> ' + languageStrings[browserLanguage].title + ' </h2><a id="usersurvey-close-window" class="jive-modal-close-top jive-close no-underline" href="javascript:void(0);"><span class="pull-right">&times;</span></a></span></div><div class="jive-modal-content clearfix" style="padding: 10px;"><div class="jive-standard-formblock jive-bookmark-form clearfix" style="padding-bottom: 0px;"><div class="jive-table"><table cellpadding="0" cellspacing="0" width="100%" border="0"><tr><td><b> ' + languageStrings[browserLanguage].request + ' </b></td></tr><tr></tr><tr><th colspan="2"><input id="vovici-survey" type="button" value=" ' + languageStrings[browserLanguage].acceptsurvey + ' " title=" ' + languageStrings[browserLanguage].acceptsurveyoverlay + ' " style="margin-right:5px;"><input id="vovici-survey-cancel" type="button" value=" ' + languageStrings[browserLanguage].acceptcancel + ' " title=" ' + languageStrings[browserLanguage].overlay + ' " style="margin-left:5px;"></th></tr></table></div></div></div></div><div id="lb_container_usersurvey_next_screen" style="width: 100%; top: 0px; left: 0px; right: 0px; bottom: 0px; z-index: 999; display: none"><div id="usersurvey-modal_next_screen" class="jive-modal" style="position: absolute; left: 50%; width:700px; margin-left: -354px; z-index:1000;display: block;"><div><span><h2 class="jive-modal-title" style="width: 300px"> Cisco Support Forums Survey </h2><a id="usersurvey_next_screen-close-window" class="jive-modal-close-top jive-close no-underline" href="javascript:void(0);"><span class="pull-right">&times;</span></a></span></div><div class="jive-modal-content clearfix" style="padding: 10px;"><div class="jive-standard-formblock jive-bookmark-form clearfix" style="padding-bottom: 0px;"><IFRAME src="' + voviciSurveyLink + '" width="100%" height="850"></IFRAME></div></div></div></div></div>');

    $("div#lb_container_usersurvey").show();
    $("div#lb_overlay").show();

    var heightFromTop = getHeightFromTop();

    if(heightFromTop > 150){
        $("div#usersurvey-modal").css({top : heightFromTop + 60});
    }

    else{
        $("div#usersurvey-modal").css({top : 150});
    }

    $('input#vovici-survey').click(function() {
      /*when user accepts the survey set user completed cookie */
      $.cookie('survey_date', timestamp, {path: '/', expires: cookieLifetime});

      if(surveyRenderNewWindow === false){
        $("div#usersurvey-modal").hide();
        $("div#lb_overlay").hide();
        $("div#lb_container_usersurvey_next_screen").show();

        var heightFromTop = getHeightFromTop();

        $("a#usersurvey_next_screen-close-window").click(function(){
          closeSurveyWindow();
        });

        if(heightFromTop > 150) {
          $("div#usersurvey-modal_next_screen").css({top : heightFromTop + 60});
        }

        else {
          $("div#usersurvey-modal_next_screen").css({top : 150});
        }
      }
      else{
        closeSurveyWindow();
        /* open the survey in a new window*/
        window.open(voviciSurveyLink, 'ciscoSurveyWindow');
      }

      return false;
    });

    $('input#vovici-survey-cancel').click(function(){
      /*the user has declined the survey, save this info in cookie*/
      $.cookie('survey_decline', timestamp, {path: '/', expires: cookieLifetime});

      $("div#lb_container_usersurvey").hide();
      $("div#lb_overlay").hide();
    });

    $("a#usersurvey-close-window").click(function(){
      /* when user clicks the Later or close link, this should set cookie and hide overlay just like vovici-survey-cancel */
      $.cookie('survey_decline', timestamp, {path: '/', expires: cookieLifetime});
  ...
  Drupal.behaviors.ciscoCoreSurvey = {
    attach: function(context, settings) {
      ...
      if (typeof(settings.cisco_core.popupDelay) != 'undefined') {
        idleTime = parseInt(settings.cisco_core.popupDelay);
        if (survey_debug === true){
          console.log(settings.cisco_core);
        }
      }

      // Set timer to display survey.
      setTimeout(
        function() {
          startSurvey(context, settings);
        },
        idleTime
      );
    }
